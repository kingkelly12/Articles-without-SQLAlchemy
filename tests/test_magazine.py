...
def test_magazine_contributing_authors(setup_db):
    magazine = Magazine.find_by_name("Test Magazine")
    article4 = Article(title="Test Article 4", author_id=1, magazine_id=magazine.id)
    article4.save()
...
def test_magazines_with_multiple_authors(setup_db):
    ...
    magazine2.save()
    article4 = Article(title="Single Author Article", author_id=1, magazine_id=magazine2.id)
    article4.save()
